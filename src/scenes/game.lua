...
local objectSheet =
	graphics.newImageSheet("src/images/gameObjects.png", sheetOptions)
...
local function createAsteroid()
	...
	physics.addBody(newAsteroid, "dynamic", {
		radius = 40,
		bounce = 0.8
	})
...
local function endGame()
	...
	composer.gotoScene("src.scenes.highscores", {
		time = 800,
		effect = "crossFade"
	})
end
...
function scene:create(event)
	...
	-- Load the background
	local background =
		display.newImageRect(backGroup, "src/images/background.png", 800, 1400)
	...
	explosionSound = audio.loadSound("src/audio/explosion.wav")
	fireSound = audio.loadSound("src/audio/fire.wav")
	musicTrack = audio.loadStream("src/audio/80s-Space-Game_Looping.wav")
end
